...
class BaseLevel(arcade.Window):
    ...
    def setup(self):
        ...
        cards_config = self.get_cards_config()
        self.initial_cards_data = cards_config.copy()
        self.total_cards = len(cards_config)
    ...
    def load_cards(self):
        ...
        for card_pos in self.get_cards_config():
            x, y = card_pos
            card = Card(x, y)
    ...
    def load_enemies_from_spawn_points(self):
        ...
        if self.spawn_entities_list:
            attack_cooldown, damage = self.get_enemy_config()
            for spawn in self.spawn_entities_list:
                enemy_x = spawn.center_x
                enemy_y = spawn.center_y
                from enemy_config import ENEMY_Y_OFFSET
    ...
    def check_game_completion(self):
        ...
        if all_cards_collected and all_enemies_dead and time_bonus:
            self.stars_earned = 3
            print(f"★ ★ ★ - ИДЕАЛЬНО! Уровень {self.get_level_number()} пройден!")
        ...
        elif all_cards_collected and (all_enemies_dead or time_bonus):
            self.stars_earned = 2
            print(f"★ ★ - Отлично! Уровень {self.get_level_number()} пройден!")
            print(f"Собрано карт: {self.cards_collected}/{self.total_cards}")
            if all_enemies_dead:
                print(f"Убито врагов: Все!")
            ...
            self.stars_earned = 1
            print(f"УРОВЕНЬ {self.get_level_number()} ПРОЙДЕН!")
            print("★ - Хорошо!")
            print(f"Собрано карт: {self.cards_collected}/{self.total_cards}")
